...
// ! 클로저 사용 이유 
// ? 1. 전역 오염 막기 위해  
// ? 2. 값을 기억하기 위해(가비지 컬렉터가 수집하는 것 막음) 
// ? 3. 내가 설정한 값을 함부로 값을 수정할 수 없게(특정 함수를 통해서만 접근 가능하게 하려고)
/*
let count = 0;    // => 전역이 오염된다. => 해결방법 -> 클로저 사용하기
...
function first(){
  ...
  function second(){
    ...
    return x + y;    // 15 => 함수를 실행해야 우리가 결과값을 받을 수 있다.
  }

  return second   // 함수 본문이 튀어나온다.
}
 
first()()    // ? 15 -> 클로저 사용하기
const value = first()   // second 함수 본문이 value에 담김
// ? value()로 second 함수 본문을 실행시켜 값을 가져오기

// second함수를 마지막으로 실행해서 first는 return 15하기 때문에 결과값이 15가 된다.
